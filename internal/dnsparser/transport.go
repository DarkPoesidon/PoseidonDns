...
// maxNULLAnswerRdata is the largest NULL-record rdata we will emit in a single
// answer. Chosen so the whole response (header + question + answer + OPT) stays
// well under a typical EDNS=4096 budget. Frames larger than this fall back to
// TXT-style chunking regardless of the configured record type.
const maxNULLAnswerRdata = 3500

func BuildVPNResponsePacket(questionPacket []byte, answerName string, packet VpnProto.Packet, baseEncode bool) ([]byte, error) {
	return BuildVPNResponsePacketWithType(questionPacket, answerName, packet, baseEncode, Enums.DNS_RECORD_TYPE_TXT)
}

// BuildVPNResponsePacketMirroringQuestion inspects the first question in
// questionPacket and emits the response using the same record type the client
// asked for. Falls back to TXT when the question's qtype is unrecognised or
// when extraction fails. This lets the server stay stateless on record-type
// negotiation: it just answers in kind.
func BuildVPNResponsePacketMirroringQuestion(questionPacket []byte, answerName string, packet VpnProto.Packet, baseEncode bool) ([]byte, error) {
	qType := extractFirstQuestionQType(questionPacket)
	if qType == 0 {
		qType = Enums.DNS_RECORD_TYPE_TXT
	}
	return BuildVPNResponsePacketWithType(questionPacket, answerName, packet, baseEncode, qType)
}

func extractFirstQuestionQType(packet []byte) uint16 {
	...
	return binary.BigEndian.Uint16(packet[nextOffset : nextOffset+2])
}

// BuildVPNResponsePacketWithType is the qtype-aware variant. When qType is
// DNS_RECORD_TYPE_NULL and the encoded frame fits in a single NULL rdata,
// the response is emitted as one NULL answer (opaque rdata, no TXT length
// prefixing) for higher throughput per byte. For any other qType, or for
// frames that exceed maxNULLAnswerRdata, the TXT-chunked path is used.
func BuildVPNResponsePacketWithType(questionPacket []byte, answerName string, packet VpnProto.Packet, baseEncode bool, qType uint16) ([]byte, error) {
	...
	if qType == Enums.DNS_RECORD_TYPE_NULL && len(rawFrame) > 0 && len(rawFrame) <= maxNULLAnswerRdata {
		return buildSingleAnswerResponsePacket(questionPacket, answerName, rawFrame, Enums.DNS_RECORD_TYPE_NULL)
	}
...
// buildSingleAnswerResponsePacket writes one resource record of the given
// answerType with rdata = answerRData (verbatim, no inner length-prefix). Used
// for NULL responses where the rdata is opaque and we want to avoid the
// 255-byte-per-string TXT chunking overhead.
func buildSingleAnswerResponsePacket(questionPacket []byte, answerName string, answerRData []byte, answerType uint16) ([]byte, error) {
	if len(questionPacket) < dnsHeaderSize {
		return nil, ErrPacketTooShort
	}
	...
	offset += copy(response[offset:], questionBytes)
	offset += copy(response[offset:], nameBytes)
	binary.BigEndian.PutUint16(response[offset:offset+2], answerType)
	binary.BigEndian.PutUint16(response[offset+2:offset+4], Enums.DNSQ_CLASS_IN)
	binary.BigEndian.PutUint32(response[offset+4:offset+8], 0)
...
func extractTXTAnswerPayloads(parsed Packet) [][]byte {
	...
	for _, answer := range parsed.Answers {
		...
		case Enums.DNS_RECORD_TYPE_NULL:
			if len(answer.RData) == 0 {
				continue
			}
			rd := make([]byte, len(answer.RData))
			copy(rd, answer.RData)
