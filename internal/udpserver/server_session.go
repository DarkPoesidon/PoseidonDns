...
func (s *Server) buildInvalidSessionErrorResponse(questionPacket []byte, requestName string, sessionID uint8, responseMode uint8) []byte {
	...
	response, err := DnsParser.BuildVPNResponsePacketMirroringQuestion(questionPacket, requestName, VpnProto.Packet{
		SessionID:  sessionID,
...
func (s *Server) buildSessionVPNResponse(questionPacket []byte, requestName string, record *sessionRuntimeView, packet VpnProto.Packet) []byte {
	...
	response, err := DnsParser.BuildVPNResponsePacketMirroringQuestion(questionPacket, requestName, packet, record.ResponseBase64)
	if err != nil {
		return nil
	}
	return response
...
func (s *Server) handleSessionInitRequest(questionPacket []byte, decision domainMatcher.Decision, vpnPacket VpnProto.Packet) []byte {
	...
	response, err := DnsParser.BuildVPNResponsePacketMirroringQuestion(questionPacket, decision.RequestName, VpnProto.Packet{
		SessionID:  0,
