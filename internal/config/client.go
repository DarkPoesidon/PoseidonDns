...
type ClientConfig struct {
	...
	EDnsUDPPayloadSize                    int               `toml:"EDNS_UDP_PAYLOAD_SIZE"`
...
func finalizeClientConfig(cfg ClientConfig) (ClientConfig, error) {
	...
	cfg.DNSRecordType = strings.ToUpper(strings.TrimSpace(cfg.DNSRecordType))
	if cfg.DNSRecordType == "" {
		cfg.DNSRecordType = "TXT"
	}
	if _, _, err := ResolveDNSRecordType(cfg.DNSRecordType); err != nil {
		return cfg, err
	}

	if cfg.EDnsUDPPayloadSize < 0 {
		return cfg, fmt.Errorf("EDNS_UDP_PAYLOAD_SIZE cannot be negative")
	}
	if cfg.EDnsUDPPayloadSize > 0 && cfg.EDnsUDPPayloadSize < 512 {
		cfg.EDnsUDPPayloadSize = 512
	}
	if cfg.EDnsUDPPayloadSize > 4096 {
		cfg.EDnsUDPPayloadSize = 4096
	}
