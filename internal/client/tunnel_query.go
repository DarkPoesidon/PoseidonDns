...
func (c *Client) effectiveTunnelQType() uint16 {
	if c.tunnelQType == 0 {
		return Enums.DNS_RECORD_TYPE_TXT
	}
	return c.tunnelQType
}

func (c *Client) effectiveTunnelEDnsUDPSize() uint16 {
	return c.tunnelEDnsUDPSize
}

func (c *Client) buildTunnelTXTQuestionBytes(domain string, encoded []byte) ([]byte, error) {
	return DnsParser.BuildTunnelTXTQuestionPacket(domain, encoded, c.effectiveTunnelQType(), c.effectiveTunnelEDnsUDPSize())
}
...
func (c *Client) buildTunnelTXTQueryRaw(domain string, options VpnProto.BuildOptions) ([]byte, error) {
	...
	return c.buildTunnelTXTQuestionBytes(domain, encoded)
}
