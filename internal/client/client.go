...
func New(cfg config.ClientConfig, log *logger.Logger, codec *security.Codec) *Client {
	...
	qtype, native, qErr := config.ResolveDNSRecordType(cfg.DNSRecordType)
	if qErr != nil {
		if log != nil {
			log.Warnf("invalid DNS_RECORD_TYPE %q, falling back to TXT: %v", cfg.DNSRecordType, qErr)
		}
	...
	if !native {
		if log != nil {
			log.Warnf("DNS_RECORD_TYPE=%s is not yet supported by the transport; downgrading to TXT", cfg.DNSRecordType)
		}
		qtype = Enums.DNS_RECORD_TYPE_TXT
	}
	c.tunnelQType = qtype
	...
	if cfg.EDnsUDPPayloadSize > 0 {
		c.tunnelEDnsUDPSize = uint16(cfg.EDnsUDPPayloadSize)
	} else {
		c.tunnelEDnsUDPSize = 0
	}
