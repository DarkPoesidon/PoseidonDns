...
func (c *Client) buildPlannedOutboundFrames(
	...
	for _, resolverConn := range conns {
		...
		case firstDNSPacket == nil:
			dnsPacket, err = DnsParser.BuildTunnelTXTQuestionPacketPrepared(prepared.normalized, prepared.qname, encoded, c.effectiveTunnelQType(), c.effectiveTunnelEDnsUDPSize())
			if err != nil {
				continue
			}
			firstDomain = domain
