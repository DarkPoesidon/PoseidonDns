...
# ------------------------------------------------------------------------------
# DNS record type + EDNS preferences.
#
# The server mirrors whatever DNS record type the client queries it with, so
# these prompts collect *client-side* recommendations. They are written to
# client_recommended_settings.txt so the operator can ship the values to
# clients out-of-band. Skipping (Enter on each) keeps the defaults: TXT + 4096.
# ------------------------------------------------------------------------------

USER_RECORD_TYPE=""
USER_EDNS_SIZE=""

echo -e "${YELLOW}${BOLD}DNS Record Type${NC} (used by clients for tunnel queries)"
echo "  1) TXT   - Standard, most compatible (default)"
echo "  2) NULL  - Opaque rdata, higher payload density per record"
echo "  3) CNAME - Reserved (clients will downgrade to TXT with a warning)"
read -r -p ">>> Choose 1/2/3 [1]: " RECORD_CHOICE </dev/tty || true
case "${RECORD_CHOICE:-1}" in
  2) USER_RECORD_TYPE="NULL" ;;
  3) USER_RECORD_TYPE="CNAME" ;;
  *) USER_RECORD_TYPE="TXT" ;;
esac

echo -e "${YELLOW}${BOLD}EDNS0${NC} (Extension Mechanisms for DNS)"
echo "  Enabling EDNS0 allows UDP responses up to 4096 bytes (recommended)."
echo "  Disable it only if your network drops EDNS-bearing DNS packets."
read -r -p ">>> Enable EDNS0 with 4096-byte payload? [Y/n]: " EDNS_CHOICE </dev/tty || true
case "${EDNS_CHOICE:-Y}" in
  n|N|no|NO) USER_EDNS_SIZE="0" ;;
  *)         USER_EDNS_SIZE="4096" ;;
esac

cat > client_recommended_settings.txt <<EOF
# ==============================================================================
# Client-side settings recommended by this server's operator.
# Copy these into each client's client_config.toml (section 1).
# Generated $(date -u +"%Y-%m-%dT%H:%M:%SZ")
# ==============================================================================

DNS_RECORD_TYPE       = "${USER_RECORD_TYPE}"
EDNS_UDP_PAYLOAD_SIZE = ${USER_EDNS_SIZE}
EOF

log_info "Client-side recommendations written to: client_recommended_settings.txt"
log_info "  DNS_RECORD_TYPE       = ${USER_RECORD_TYPE}"
log_info "  EDNS_UDP_PAYLOAD_SIZE = ${USER_EDNS_SIZE}"
